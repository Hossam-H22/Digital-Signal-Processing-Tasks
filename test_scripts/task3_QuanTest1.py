def QuantizationTest1(file_name,Your_EncodedValues,Your_QuantizedValues):
    ...
    for i in range(len(Your_EncodedValues)):
        if(Your_EncodedValues[i]!=expectedEncodedValues[i]):
            return("QuantizationTest1 Test case failed, your EncodedValues have different EncodedValues from the expected one")
    for i in range(len(expectedQuantizedValues)):
        if abs(Your_QuantizedValues[i] - expectedQuantizedValues[i]) < 0.01:
            continue
        else:
            return("QuantizationTest1 Test case failed, your QuantizedValues have different values from the expected one")
